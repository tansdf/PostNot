...
pub(crate) type DynAsyncUnitCallback =
    Box<dyn for<'a> FnMut() -> BoxFuture<'static, ()> + 'static + Send + Sync>;
...
impl Deref for Callback<DynAsyncUnitCallback> {
    type Target = dyn for<'a> FnMut() -> BoxFuture<'static, ()> + 'static + Sync + Send;
...
impl DerefMut for Callback<DynAsyncUnitCallback> {
    fn deref_mut(&mut self) -> &mut Self::Target {
        self.inner.as_mut()
    }
}

impl Callback<DynAsyncUnitCallback> {
    pub(crate) fn new<T>(callback: T) -> Self
    where
        T: for<'a> FnMut() -> BoxFuture<'static, ()> + 'static + Sync + Send,
    {
        Callback {
            inner: Box::new(callback),
        }
