...
pub struct ClientBuilder {
    ...
    pub(crate) on_reconnect_failed: Option<Callback<DynAsyncUnitCallback>>,
    pub(crate) on_connection_closed: Option<Callback<DynAsyncUnitCallback>>,
...
impl ClientBuilder {
    ...
    /// Registers a callback invoked after the configured reconnect attempts are
    /// exhausted without establishing a new connection.
    pub fn on_reconnect_failed<F>(mut self, callback: F) -> Self
    where
        F: for<'a> std::ops::FnMut() -> BoxFuture<'static, ()> + 'static + Send + Sync,
    {
        self.on_reconnect_failed = Some(Callback::<DynAsyncUnitCallback>::new(callback));
        self
    }

    /// Registers a callback invoked when the connection stream ends and no
    /// reconnect will be attempted.
    pub fn on_connection_closed<F>(mut self, callback: F) -> Self
    where
        F: for<'a> std::ops::FnMut() -> BoxFuture<'static, ()> + 'static + Send + Sync,
    {
        self.on_connection_closed = Some(Callback::<DynAsyncUnitCallback>::new(callback));
        self
    }
