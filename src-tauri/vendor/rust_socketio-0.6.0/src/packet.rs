...
impl Packet {
    ...
    pub(crate) fn new_from_payload<'a>(
        ...
        match payload {
            // An ACK id on an outgoing binary event requests an acknowledgement;
            // it does not turn the event itself into a BinaryAck packet. Socket.IO
            // servers ignore a client-originated BinaryAck without a matching event.
